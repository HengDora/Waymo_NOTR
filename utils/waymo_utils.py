...
def get_obj_pose_tracking(datadir, selected_frames, ego_poses, cameras=[0, 1, 2, 3, 4]):
    ...
    print(f'Loading from，正在从此加载 : {tracklet_path}')
    ...
    print("Removing static objects，正在移除静态对象。")
    ...
    print("Max obj per frame，每帧最大对象数:", max_obj_per_frame_new)

    if max_obj_per_frame_new == 0:
        print("No moving obj in current sequence, make dummy visible objects，当前序列中没有移动对象，制作虚拟可见对象。")
    ...
    print('box scale，边界框尺度: ', box_scale)
...
def generate_dataparser_outputs(
    ...
    # 导入相机内参、外参和ego，这些数据都是由waymo数据集提供的，一键生成。
    ...
    # run colmap
    # colmap在这里是用来做sfm的，这里是用waymo数据集的点云来做sfm
    # 如果前面
    ...
    if build_pointcloud:
        print('build point cloud，已经build了点云。')
        ...
        print('initialize from sfm pointcloud，从sfm点云初始化')
        ...
        print('initialize from lidar pointcloud，从LiDAR点云初始化')
        ...
        # 获取得到球的中心和半径
        # 计算点云的包围球(lidar_sphere_normalization)，用于后续坐标归一化。
        ...
        # 将SfM点云与LiDAR点云结合
        # 通过filter_colmap配置过滤离相机过远的点（依赖cfg.data.extent参数）
        try:
            ...
            print('No colmap pointcloud，没有colmap点云')
        ...
        for k in points_xyz_dict.keys():
            ...
            try:
                ...
                print(f'正在为 {k} 物体保存点云, 初始的点云点数量是 {points_xyz.shape}')
            except:
                print(f'failed to save pointcloud for， 为 {k} 创建点云失败')
                continue
    return result